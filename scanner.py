...
def ARPSpoofingDetector(packetARP, packetNumber):
    ...
    if packetARP.spa in macToIP.keys() and packetARP.sha != macToIP[packetARP.spa]:
        print "ARP spoofing!  MAC:{}  Packet number:{} ".format(formatMac(packetARP.sha) ,packetNumber)
        ...
        for dest in portScan:
            ...
            for src in source:
                ...
                if (len(arrayOfPort) > 100):
                    print "\n"
                    print "Port scan!  IP: {}  Packet number: {}" \
                          "".format(formatIP(arrayOfPort[0]['dst']), packetsNumberArray)
        ...
        if tcpPacket.flags == dpkt.tcp.TH_SYN:
            if packetIP.dst in synFlood:
                ...
                if len(arrayOfValues) > 100:
                    ...
                    print "SYN floods!  IP: {}  Packet number: " \
                          "{}".format(formatIP(arrayOfValues[0]['dst']),arrayOfPacketNumber)
                    print "\n"
                    arrayOfValues.clear()
            else:
...
#------------------- main Script -----------------------------
fileString = raw_input("Enter the pcap file (e.g.2_attack/portscan.pcap):")
f = open(fileString)
